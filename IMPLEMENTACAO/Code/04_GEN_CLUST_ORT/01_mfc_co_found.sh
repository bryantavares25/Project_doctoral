    ...
    while read -r line; do
        ...
        mfc_coc_clean=$dir/Documentos/GitHub/Project_doctoral/IMPLEMENTACAO/Gene_clusters/M_flocculare/$line/genomic_gene_coc_clean.bed

        # Pegar cada linha e descobrir operon
        while IFS=$'\t' read -r col1 col2 col3 col4; do
            awk -v line="$line" -v col1="$col1" -v col2="$col2" -v col3="$col3" -v col4="$col4" -F'\t' '($1 ~ line) && ($0 ~ col4) {print col2, col1, col3, col4, $2, $3, $5, $4}' OFS='\t' $mfc_coc_clean >> $mfc_after_operon
        done < $mfc_after
